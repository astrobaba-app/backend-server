...
const sendAstrologerApprovalEmail = async (astrologer) => {
  try {
    await transporter.sendMail({
      ...
      to: astrologer.email,
      subject: "Your Graho Profile Has Been Approved! ",
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #4CAF50;">Congratulations ${astrologer.fullName}!</h2>
          <p>Your astrologer profile on Graho has been approved.</p>
          ...
          <p>Best regards,<br>Graho Team</p>
        </div>
      `,
    });
    return { success: true };
...
const sendAstrologerRejectionEmail = async (astrologer, reason = null) => {
  try {
    await transporter.sendMail({
      from: process.env.ADMIN_EMAIL,
      to: astrologer.email,
      subject: "Update on Your Graho Application",
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #f44336;">Application Status Update</h2>
          <p>Dear ${astrologer.fullName},</p>
          <p>Thank you for your interest in joining Graho as an astrologer.</p>
